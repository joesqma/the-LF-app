...
import { gettingFasterLessons } from "~/lib/content/getting-faster";
...
export default async function GettingFasterLessonPage({ params }: Props) {
  const { lessonId } = await params;

  const lessonIndex = gettingFasterLessons.findIndex((l) => l.id === lessonId);
  if (lessonIndex === -1) notFound();
  const lesson = gettingFasterLessons[lessonIndex];
  ...
  const nextLesson = gettingFasterLessons[lessonIndex + 1] ?? null;
  const nextLessonHref = nextLesson
    ? `/learn/getting-faster/${nextLesson.id}`
    : null;

  return (
    <PageShell
      title={lesson.title}
      subtitle={`${lesson.estimatedMinutes} min · Getting Faster · ${lesson.phase}`}
    >
